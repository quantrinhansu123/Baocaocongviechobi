...
function readEnv() {
  ...
  if (!process.env.VERCEL) {
    import_dotenv.default.config({ path: import_node_path.default.join(process.cwd(), ".env"), override: true });
  }
  return process.env;
}
...
// api-src/routes/delete.ts
var delete_default = withJsonApiHandler(async (req, res) => {
